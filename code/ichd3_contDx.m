...
function [ICHD3] = ichd3_contDx(tbl)
...
%% Migraine features
        ...
        % determine migraine score, 3 is migraine, 2 is probable migraine,
        % only based on B-D diagnostic criteria because this continuous
        % headache
        ...
        ICHD3.migraine(ICHD3.mig_score==3) = 1;
        
        ICHD3.probable_migraine = zeros(height(tbl),1);
        ICHD3.probable_migraine(ICHD3.mig_score==2) = 1;
        ...
        ICHD3.chronic_migraine((ICHD3.migraine==1 | ICHD3.probable_migraine==1) & (tbl.p_fre_bad=='2to3wk'|tbl.p_fre_bad=='3wk'|tbl.p_fre_bad=='daily'|tbl.p_fre_bad=='always')) = 1;
        ...
        % determine chronic tension-type headache score, only based on
        % criteria B-D
        ICHD3.tth_score = zeros(height(tbl),1);
        
        for x=1:height(tbl)
            if ICHD3.mig_dur(x)==1 % criteria B of tth ICHD3 of headache lasting 30 min to days
        ...
        ICHD3.chronic_tth(ICHD3.tth_score==3) = 1;
        ...
        ICHD3.hemicrania_continua = zeros(height(tbl),1);
        ICHD3.hemicrania_continua(ICHD3.unilateral_sideLocked==1 & ICHD3.unilateral_autonomic==1) = 1;
